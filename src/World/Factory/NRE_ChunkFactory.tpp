     ...
     namespace NRE {
         namespace World {
             namespace Factory {
                 ...
                 inline void createTerrain(Chunk& target, int worldSizeY, Math::Vector3D<float> const&) {
                     ...
                     for (std::size_t z = 0; z <= Chunk::SIZE_Z; ++z) {
                         ...
                         for (std::size_t x = 0; x <= Chunk::SIZE_X; ++x) {
                             ...
                             float biasedE = e * Chunk::SIZE_Y * static_cast <float> (worldSizeY);
                             ...
                             for (std::size_t y = 0; y <= Chunk::SIZE_Y; ++y) {
                                 ...
                                 if (e < 0.18f) {
                                     target[index].setIsoValue(ny - 0.18f * Chunk::SIZE_Y * static_cast <float> (worldSizeY) + 0.0001f);
                                 } else {
